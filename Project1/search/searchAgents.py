...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        currentPossition, visitedCorners = state
...
def cornersHeuristic(state, problem):
    ...
    for corner in range(len(corners)):
        ...
        for i in range(len(nextVisitedCorners)):
            distToCorners[i] = util.manhattanDistance(currentPosition, corners[i])
        nearestCorner = 0
        for corner in range(len(distToCorners)):
            if (nextVisitedCorners[nearestCorner]):
                nearestCorner = corner
            if (not nextVisitedCorners[corner]) and (distToCorners[corner] < distToCorners[nearestCorner]):
                nearestCorner = corner
        if (not nextVisitedCorners[nearestCorner]):
            currentPosition = corners[nearestCorner]
            nextVisitedCorners[nearestCorner] = True
            result += distToCorners[nearestCorner]
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
        totalActions = search.aStarSearch(problem, search.nullHeuristic)
        return totalActions
